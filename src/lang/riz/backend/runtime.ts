import { LiquidErrorInstance } from '$core/error';
...
class Scope {
  ...
  public declare(name: string) {
    if (this.variables.has(name)) {
      throw new LiquidErrorInstance('Runtime', `Variable '${name}' is already declared`);
    }
    ...
    switch (true) {
      ...
      case expression instanceof Riz.Expression.Identifier:
        if (!this._find(expression.name)) {
          throw new LiquidErrorInstance('Runtime', `Variable '${expression.name}' is not declared`);
        }
